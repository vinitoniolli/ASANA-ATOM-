...
export function ComercialPage() {
  const { data, status, error, isRefreshing, refresh } = useDashboardData(
    'https://n8nsemfila.iatom.site/webhook/e54127cc-6000-4349-aeec-6d6dd1851cbf',
  )
  ...
  if (status === 'loading' && !data) {
    return (
      <section className="page-shell">
        <Header panelName="ATOM" sectionName="Comercial" status="loading" />
        <LoadingState />
      </section>
    )
  }
  ...
  if (status === 'error' && !data) {
    return (
      <section className="page-shell">
        <Header panelName="ATOM" sectionName="Comercial" status="error" onRefresh={refresh} />
        <ErrorState
